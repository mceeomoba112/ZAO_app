...
// Load friends list with online status
    ...
    try {
        ...
        if (friendships && friendships.length > 0) {
            ...
            const { data: friendProfiles, error: profileError } = await supabase
                .from('user_profiles')
                .select('full_name, username, avatar_url, user_id, is_online, last_seen')
...
// Create user element with online status indicators
function createUserElement(user, type) {
    ...
    element.innerHTML = `
        ...
        <div class="user-details">
            ...
            ${user.is_online ? '<div class="online-text">Online</div>' : '<div class="offline-text">Offline</div>'}
        </div>
    `;
...
// Subscribe to real-time messages with enhanced synchronization
    ...
    messagesSubscription = supabase
        ...
        .on(
            ...
            async (payload) => {
                ...
                // Display message if it's relevant to current user or active chat
                const isRelevantMessage = 
                    (payload.new.user_id === currentUser.id) || 
                    (payload.new.recipient_id === currentUser.id);

                if (isRelevantMessage) {
                    // If we have an active chat, check if message belongs to it
        ...
        .on(
            ...
            (payload) => {
                // Update online status when user profiles change
                updateOnlineStatus(payload.new);
            }
        )
        .subscribe();

    // Set up auto-refresh every 3 seconds for message sync
    setupAutoRefresh();
}

// Auto-refresh functionality for message synchronization
...
function updateOnlineStatus(userProfile) {
